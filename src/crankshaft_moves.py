...
class crankshaft_moves(movement.movement):
    ...
    def mutation(self, structure_grid):
        """ Search if the residu i is part of a u-shaped then search the
        rotation
        ...
        while i < 4:
            ...
            if getattr(self.residu, attribut[i]) == getattr(next, attribut[i]):
                ...
                if (getattr(previous, attribut[i]) ==
                    ...
                    #       i      i+1
                    #       i-1    i+2
                    if (i == 0 and structure_grid[getattr(previous,
                        attribut2[i]) + delta][getattr(self.residu,
                        attribut[i])] == -1 and
                       structure_grid[getattr(following, attribut2[i]) + delta]
                       [getattr(next, attribut[i])] == -1):
                        check_conformation = True

                    # Conformation vertical:
                    #       i-1      i
                    #       i+2     i+1
                    elif (i == 1 and structure_grid[getattr(self.residu,
                          attribut[i])][getattr(previous, attribut2[i]) +
                          delta] == -1 and
                          structure_grid[getattr(next, attribut[i])][
                          getattr(following, attribut2[i]) + delta] == -1):
                        check_conformation = True

                    # Conformation horizontal:
                    #       i-1     i
                    #       i-2    i+1
                    elif (i == 2 and structure_grid[getattr(next,
                          attribut2[i]) + delta][getattr(self.residu,
                          attribut[i])] == -1 and
                          structure_grid[getattr(following, attribut2[i]) +
                          delta][getattr(previous, attribut[i])] == -1):
                        check_conformation = True

                    # Conformation vertical:
                    #       i-2     i-1
                    #       i+1      i
                    elif (i == 3 and structure_grid[getattr(self.residu,
                          attribut[i])][getattr(next, attribut2[i]) +
                          delta] == -1 and structure_grid[getattr(previous,
                          attribut[i])][getattr(following, attribut2[i]) +
                          delta] == -1):
                        check_conformation = True

            if check_conformation:
                if i < 2:
