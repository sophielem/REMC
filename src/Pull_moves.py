...
class Pull_moves(Movement.Movement):
    ...
    def __findNeighbourOccupied(self, structure_grid, j, idx, elmt, i):
        """ Find if a neighbour of the first or last residues of sequence of 3 residues is occupied or not
            in the next or previous column or next or previous line
        """
        second_residu = None
        for offset in [1, -1]:
            if i == 0:
                if structure_grid[j][idx + offset] == elmt[j] + offset:
                    ...
                    second_residu = elmt[j]
                    return [a, second_residu]
                elif structure_grid[j+2][idx + offset] == elmt[j+2] + offset:
                    a = elmt[j+2] + offset
                ...
                print(elmt[j+2] + offset)
                print(structure_grid[idx + offset][j+2])
                print(structure_grid[idx - offset][j+2])
                if structure_grid[idx + offset][j] == elmt[j] + offset:
                    ...
                    second_residu = elmt[j]
                    return [a, second_residu]
                elif structure_grid[idx + offset][j+2] == elmt[j+2] + offset:
        ...
        if i == 0:
            if a.line == j:
                if structure_grid[a.line + 1][a.column] == -1 and structure_grid[a.line + 2][a.column] == -1:
                    ...
                    return [{'line': a.line + 2, 'column': a.column}, {'line': a.line + 1, 'column': a.column}, second_residu - self.index]
            else:
                if structure_grid[a.line - 1][a.column] == -1 and structure_grid[a.line - 2][a.column] == -1:
                    return [{'line': a.line - 2, 'column': a.column}, {'line': a.line - 1, 'column': a.column}, second_residu - self.index]
        else:
            if a.column == j:
                if structure_grid[a.line][a.column + 1] == -1 and structure_grid[a.line][a.column + 2] == -1:
                    # Les 2 emplacements vides, et le deuxieme residu à bouger
                    return [{'line': a.line, 'column': a.column + 2}, {'line': a.line, 'column': a.column + 1}, second_residu - self.index]
            else:
                if structure_grid[a.line][a.column - 1] == -1 and structure_grid[a.line][a.column - 2] == -1:
                    # Les 2 emplacements vides, et le deuxieme residu à bouger
                    return [{'line': a.line, 'column': a.column - 2}, {'line': a.line, 'column': a.column - 1}, second_residu - self.index]
    ...
    def mutation(self, structure_grid, residues):
        ...
        while i < 2 and flag == False:
            ...
            # Then, check if it has 2 neighbours occupied in the same line or column
        ...
        if flag:
            ...
            a, second_residu = self.__findNeighbourOccupied(structure_grid, j, idx, elmt, i)
            for l in structure_grid:
                for m in l:
                    print("{0:2d}".format(m), end='')

                print("")
            if a is not None:
                print(residues[a].line, residues[a].column)
                # Check if residu a has 2 free neighbours
                return self.__checkNeighboursFree(structure_grid, residues[a], j, second_residu, i)
